...
static void level_player_draw_power_animation(struct player_t *player, struct object_t *obj);
...
static void level_player_die(struct player_t *player) {
	...
	level_player_draw_death_animation(player);
...
static void level_update_palette() {
	...
	if (g_vars.level_time < 5) {
		...
		g_sys.set_screen_palette(palette_buffer, 128, 128);
...
static uint8_t level_lookup_tile(uint8_t num) {
	...
	_al = g_vars.tilemap_current_lut[num];
	if (_al == 248 && (g_vars.tilemap_flags & 1) != 0) {
		++_al;
	}
	return _al;
}
...
static void level_player_hit_object(struct player_t *p, struct object_t *obj) {
	if (g_vars.cheats & CHEATS_OBJECT_NO_HIT) {
		return;
	}
	...
	if (player_flags(&p->obj) & 0x20) {
		level_player_draw_death_animation(p);
	}
	level_hit_player(p);
...
static void level_update_player_anim_6(struct player_t *player) {
	if (player->vinyls_count == 0) {
		...
		if (obj) {
			...
			object22_xvelocity(obj) = (player_flags(&player->obj) & 4) ? -24 : 24;
			int ax = ((player_flags(&player->obj) & 0x20) != 0) ? 0x200 : 0x100;
...
static void level_update_player_anim_10(struct player_t *player) {
...
static void level_update_player_anim_27(struct player_t *player) {
	print_warning("Unimplemented level_update_player_anim_27");
}
...
static void level_update_player_anim_28(struct player_t *player) {
	...
	player->obj.y_pos -= 12;
	if (player_throw_counter(&player->obj) != 0 && (player_flags(&player->obj) & 0x20) != 0) {
...
static void level_update_player_from_input(struct player_t *player) {
	if (player_flags2(&player->obj) & 8) {
		if ((player_flags2(&player->obj) & 1) == 0) {
			...
			player_x_delta(&player->obj) = (player->obj.x_pos < g_vars.tilemap_x + 10) ? -24 : 24;
	...
	if (player_flags(&player->obj) & 0x8) {
		++player_power(&player->obj);
	} else {
		player_power(&player->obj) = 0;
	}
	const uint8_t flags = player_flags(&player->obj);
	if (g_vars.input_key_right) {
		player_flags(&player->obj) &= ~4;
	}
	...
	if (((flags ^ player_flags(&player->obj)) & 4) != 0) { // horizontal direction changed
		player->change_hdir_counter += 4;
	}
	uint8_t _ah = player_flags(&player->obj);
	uint8_t _al = 0;
	uint8_t _dh = _ah;
	uint8_t _dl = _ah;
	_ah >>= 1;
	...
	case 9:
		...
		break;
	case 10:
		level_update_player_anim_10(player);
		break;
	case 11:
	...
	case 28:
		...
		break;
	default:
		print_warning("Unhandled anim_lut %d _al %d", player_anim_lut[_al], _al);
		break;
	}
...
static void level_init_object102_1(struct object_t *si, int x, int y, int num) {
	...
	if (obj) {
		...
		object102_x_delta(obj) = (object102_x_delta(obj) < 0) ? -n : n;
		object102_y_delta(obj) = -(((get_random_number() & 7) + 4) << 3);
	}
}

static void level_init_object102_2(int x, int y) {
	struct object_t *obj = find_object(102, 10, 1);
	if (obj) {
		obj->x_pos = x + 8;
		obj->y_pos = y + 12;
		...
		object_blinking_counter(obj) = 0;
		object102_x_delta(obj) = (object102_x_delta(obj) < 0) ? -16 : 16;
...
static int level_update_object82_helper(struct object_t *obj, int8_t al, int8_t dl) {
	int bp = 0;
	obj->data[3].b[0] = al;
	obj->x_pos += al >> 3;
	if (obj->x_pos < 0) {
...
static void level_update_object82_type0(struct object_t *obj) {
	if (object82_pos_data(obj) != monster_pos_data0 && !level_is_object_visible(obj->x_pos, obj->y_pos)) {
		...
		if (state == 0) {
			...
			if ((player_flags2(&player->obj) & 0x40) != 0) {
				...
				if ((player->dir_mask & 4) == 0) {
					...
					if ((player->dir_mask & 1) == 0) {
						x_pos += 60;
						if ((player_flags(&player->obj) & 4) != 0) {
							x_pos -= 20;
						} else {
			...
			if (level_update_object82_helper(obj, object82_type0_x_delta(obj), object82_type0_y_delta(obj)) != 0) {
	...
	if (!level_is_object_visible(obj->x_pos, obj->y_pos)) {
		...
		if (al == 0) {
			if (level_get_closest_player(obj, 160)) {
				object82_state(obj) = 1;
				object82_pos_data(obj) = monster_pos_data4;
				object82_counter(obj) = 40;
			}
		} else if (al == 1) {
			level_update_tiles(obj, obj->x_pos, obj->y_pos, _undefined);
			level_update_object82_helper(obj, 0, obj->data[5].b[0]);
...
static void level_update_object82_type2(struct object_t *obj) {
	if (!level_is_object_visible(obj->x_pos, obj->y_pos)) {
		...
		if (state == 0) {
			...
			if (state == 1) {
				if (obj->data[5].b[0] == 0 && --object82_counter(obj) == 0) {
		...
		if (level_update_object82_helper(obj, obj->data[4].b[0], 0) != 0) {
			obj->data[4].b[0] = -obj->data[4].b[0];
		}
update_tiles:
		level_update_tiles(obj, obj->x_pos, obj->y_pos, _undefined);
		if (level_is_object_on_tile_5dc8(obj)) {
			obj->data[4].w = -obj->data[4].w;
			if (level_update_object82_helper(obj, obj->data[4].b[0], 0) != 0) {
				obj->data[4].b[0] = -obj->data[4].b[0];
			}
		}
...
static void level_update_object82_type4_6(struct object_t *obj) {
	if (!level_is_object_visible(obj->x_pos, obj->y_pos)) {
		...
		if (state == 0) {
			...
			if (player) {
				obj->data[4].w = (obj->x_pos < player->obj.x_pos) ? 8 : -8;
				object82_state(obj) = 1;
			}
		} else if (state == 1) { // head to player
			struct player_t *player = level_get_closest_player(obj, 300);
			if (player) {
				obj->data[4].w <<= 2; // faster
				object82_pos_data(obj) = (object82_type(obj) == 4) ? (monster_pos_data6 + 10) : (monster_pos_data8 + 26);
		...
		obj->data[3].b[0] = obj->data[4].w >> 8;
		if (level_update_object82_helper(obj, obj->data[4].b[0], obj->data[5].b[0]) != 0) {
			obj->data[4].w = -obj->data[4].w;
		}
	}
}
...
static void level_update_triggers() {
	...
	for (int i = 0; i < count; ++i, data += 16, ++ptr) {
		...
		if (READ_LE_UINT16(data + 4) == 0 && READ_LE_UINT16(data + 6) == 9) {
			...
			if (num == 12 || num == 25 || num == 8) {
				if (!level_is_object_visible(READ_LE_UINT16(data), READ_LE_UINT16(data + 2))) {
					continue;
				}
				...
				if (obj) {
					...
					if (num != 8) {
						...
						if (num == 12) {
							WRITE_LE_UINT16(&g_vars.triggers_table[19] + i * 16 + 0xE, 0x260);
						}
					}
					obj[0].spr_num = dx;
					object_blinking_counter(&obj[0]) = 0;
					obj[0].data[1].w = READ_LE_UINT16(data + 0xE);
					obj[1].spr_num = 204;
				...
				if (obj) {
					...
					object82_pos_data(obj) = monster_pos_data1;
					int type = 0;
					if (num == 3) {
						obj->spr_num = 230; // dragon
						for (int i = 0; i < 128; ++i) {
							g_vars.buffer[0x101 + i] = 0xFFFF;
						}
				...
				if (obj) {
					...
					if (num == 21) {
						...
						type = 3;
						object82_pos_data(obj) = monster_pos_data4 + 18;
					} else {
				...
				if (obj) {
					...
					if (num == 17) {
						type = 4;
						object82_pos_data(obj) = monster_pos_data6;
					} else {
						type = 6;
				...
				if (obj) {
					obj->spr_num = 242;
					obj->x_pos = READ_LE_UINT16(data);
					obj->y_pos = READ_LE_UINT16(data + 2) + 16;
					obj->data[3].w = 0;
		...
		} else if (READ_LE_UINT16(data + 4) == 2 && level_is_object_visible(READ_LE_UINT16(data), READ_LE_UINT16(data + 2))) { // bonus
			const uint16_t num = READ_LE_UINT16(data + 8);
			const uint16_t ax = 190 + level_data4[0x3B0 + num];
	...
	for (int i = 0; i < 9; ++i) {
		...
		if (obj->data[0].w < 0) {
			...
			if (tiles != 0x9594 && tiles != 0) {
				level_init_object102_2(offset / g_vars.tilemap_w, offset % g_vars.tilemap_w - 2);
			}
			...
			if (tiles != 0x9695 && tiles != 0) {
				level_init_object102_2(offset / g_vars.tilemap_w, offset % g_vars.tilemap_w);
			}
	...
	for (int i = 0; i < 10; ++i) { // vinyls
		...
		if (obj2 && object_blinking_counter(obj2) == 0) {
			...
			if (num == 6 || num == 4 || num == 1) {
				ax = abs(obj2->data[4].w);
				obj2->data[4].w = (obj->data[0].w < 0) ? -ax : ax;
			} else if (num == 2) {
			...
			if (ax <= 0 || (g_vars.cheats & CHEATS_ONE_HIT_VINYL) != 0) {
				play_sound(SOUND_1);
				ax = -80;
	...
	for (int i = 0; i < 20; ++i) {
		...
		if (obj->spr_num != 0xFFFF) {
			const uint8_t *p = object82_pos_data(obj);
			int num;
			while (1) {
			...
			obj->spr_num = num | ((obj->data[3].b[0] & 0x80) << 8);
			object82_pos_data(obj) = p + 2;
...
bonus_objects:
	for (int i = 0; i < 10; ++i) { // bonuses
		...
		if (spr_num == 194) { // cake, power up
			level_player_draw_power_animation(player, obj);
		} else {
			play_sound(SOUND_10);
...
static void level_draw_objects() {
	for (int i = OBJECTS_COUNT - 1; i >= 0; --i) {
		...
		const int spr_flag = ((obj->spr_num & 0x8000) != 0) ? 1 : 0;
		const int spr_y_pos = obj->y_pos - ((g_vars.tilemap_y << 4) + g_vars.tilemap_scroll_dy) - (sprite_offsets[num] >> 8);
		int _dx = obj->x_pos - ((g_vars.tilemap_x << 4) + (g_vars.tilemap_scroll_dx << 2));
		int _cl = (sprite_sizes[num] & 255);
		if (spr_flag) {
			_cl = (sprite_offsets[num] & 255) - _cl;
		}
		const int spr_x_pos = _dx - _cl;
		video_draw_sprite(num, spr_x_pos, spr_y_pos, spr_flag);
	}
}
...
static void level_player_update_spr(struct player_t *player, struct object_t *obj) {
	...
	if ((player_flags2(&player->obj) & 1) == 0) {
		if (player->obj.spr_num != 6 && player->obj.spr_num != 19 && player_y_delta(&player->obj) > 64) {
			...
			num = (num == 32) ? 0 : 4;
			if (player_flags(&player->obj) & 0x20) {
				num |= 2;
			}
			if (player_flags(&player->obj) & 0x40) {
				num |= 1;
	...
	num &= 0x1FFF;
	num |= (player_flags(&player->obj) & 4) << 13;
	if (_dx) {
		num ^= 0x8000;
	}
	obj->spr_num = num;
...
static void level_tile_func_5dc9(struct object_t *obj, int bp) {
	...
	const int offset = (g_vars.player_yscroll >> 8) * g_vars.tilemap_w + (g_vars.player_yscroll & 255);
	uint8_t al;
...
static void level_tile_func_5eec(struct object_t *obj, int bp) {
	...
	const uint16_t bx = g_vars.player_yscroll;
	g_vars.tilemap_data[(bx >> 8) * g_vars.tilemap_w + (bx & 255)] = 0;
	++player->vinyls_count;
	level_init_object_spr_num_104(bx & 0xFF, bx >> 8);
	play_sound(SOUND_5);
	level_tile_func_5daa(obj, bp);
}
...
static void sub_15BA5(struct player_t *player) {
	...
	player_flags(&player->obj) &= ~2;
	if (!sub_15D99(player, player->obj.x_pos, player->obj.y_pos)) {
	...
	if ((player_flags(&player->obj) & 0x80) != 0) {
...
static void level_adjust_player_position() {
	if (g_vars.player_hit_counter != 0) {
...
static void level_init_players() {
	...
	const uint8_t flag = (g_vars.player & 1) << 7;
...
static void level_init_tilemap() {
	...
	while (1) {
		...
		const int _dx = g_vars.tilemap_y + 11;
		const int _ax = g_vars.players_table[0].obj.y_pos >> 4;
		if (_dx > _ax) {
			break;
		}
...
static void level_player_draw_death_animation(struct player_t *player) {
	if (player_flags(&player->obj) & 0x20) {
		...
		for (int i = 0; data[i] != 0xFF; ++i) {
			...
			if (player_flags(&player->obj) & 0x80) {
...
static void level_player_draw_power_animation(struct player_t *player, struct object_t *obj) {
	if ((player_flags(&player->obj) & 0x20) == 0) {
		play_sound(SOUND_10);
		if (player_flags(&player->obj) & 0x40) {
